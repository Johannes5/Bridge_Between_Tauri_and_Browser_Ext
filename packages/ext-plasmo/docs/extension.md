...
- **`focus.window`**: After activating or creating a tab, send a `focus.window` payload with the cached **`hwnd`** from the latest `windows.list` so the sidecar can raise the top-level browser frame via Win32.
...
- `focus.window` carries **`hwnd`**; the extension maintains `windowId` → HWND using **`windows.list`** and scored title matching.
